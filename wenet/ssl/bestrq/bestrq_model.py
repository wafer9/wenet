...
def quantize_vector_bk(latent: torch.Tensor, codebook: torch.Tensor):
    """
    Symbols in comments:
    B: batch_size.
    D: latent_dim.
...
def quantize_vector(latent: torch.Tensor, codebook: torch.Tensor):
    ...
    latent = latent.reshape(b, g, d // g)
    latent = torch.nn.functional.normalize(latent, p=2, dim=2)
    codebook = torch.nn.functional.normalize(codebook, p=2, dim=2)

    # [B, G, C]
    # torch.transpose(codebook, [2,1,0])
    distance = torch.einsum('bgd,gdc->bgc', latent, codebook.permute([1, 2, 0]))

    # [B, G]
    codes = torch.argmax(distance, dim=-1)
...
class BestRQModel(torch.nn.Module):
    ...
    def __init__(
        ...
        torch.nn.init.normal_(self.embeddings) # uniform_
    ...
    def forward(
        ...
        out, _ = self.encoder(masked_xs, xs_lens)
        ...
        num_codes = torch.tensor(target_ids.numel())
        target_ids_uniq = [torch.unique(target_ids[:,:,i]).numel() for i in range(self.num_codebooks)]
        uniq_num_codes = torch.tensor(sum(target_ids_uniq)/self.num_codebooks)
        # uniq_num_codes = torch.tensor(torch.unique(target_ids).numel()).detach()
        ids_corr = out.argmax(dim=-1, keepdim=False).transpose(1,2) == target_ids
        codes_acc = ids_corr.sum() / num_codes
        return {
            "codes_acc": codes_acc,
            "loss": loss,
            "num_codes": num_codes/self.num_codebooks,
    ...
    def _compute_loss_bk(self, input: torch.Tensor, target: torch.Tensor,
    ...
    def _compute_loss(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        loss = self.criterion(input.reshape(-1, input.size(-1)), target.reshape(-1))
        return loss
    ...
    def _nearest_embedding_idx(self, xs: torch.Tensor) -> torch.Tensor:
        ...
        codes = quantize_vector(xs_flatten, self.embeddings)
    ...
    def recognize(self,
        ...
        if self.signal_norm_var:
            xs = xs * self.signal_istd
        unmasked_xs = self._stack_features(xs)
        target_ids = self._nearest_embedding_idx(unmasked_xs)
        return target_ids
...
if __name__ == '__main__':
    ...
    encoder = ConformerEncoder(80, global_cmvn=global_cmvn,)
    model = BestRQModel(encoder)
    xs = torch.rand([8, 100, 80])
    xs_lens = torch.LongTensor([100]*8)

    batch = dict(feats=xs, feats_lengths=xs_lens)

    y = model(batch, xs.device)